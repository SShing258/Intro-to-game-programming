...
Entity::Entity() : mPosition {0.0f, 0.0f}, mMovement {0.0f, 0.0f}, 
                   ...
                   mAnimationAtlas {{}}, mAnimationIndices {}, mFrameSpeed {0}, mObjectType {STATIC} { }

Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, ObjectType type) :
    ...
    mFrameSpeed {0}, mSpeed {DEFAULT_SPEED}, mAngle {0.0f}, mObjectType {type} {}

Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        TextureType textureType, Vector2 spriteSheetDimensions, std::map<Direction, 
        std::vector<int>> animationAtlas, ObjectType type) : mPosition {position},
        mVelocity {0.0f, 0.0f}, mAcceleration {0.0f, 0.0f}, 
        ...
        mSpeed { DEFAULT_SPEED }, mObjectType {type} { }
...
void Entity::update(float deltaTime, Entity *collidableEntities, int collisionCheckCount)
{
    ...
    //Damping down the velocity
    const float dampingFactor = 0.999f;
    if (mMovement.x == 0) {
        mVelocity.x *= dampingFactor;

        if (fabs(mVelocity.x) < 0.01f) mVelocity.x = 0.0f;
    }


    if (mObjectType == MOVING) {
        mVelocity.y += 9.81 * deltaTime;
    }



    mAcceleration.x = mMovement.x * 10.0f;
    mAcceleration.y = mMovement.y * 20.0f;
