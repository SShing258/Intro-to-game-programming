...
void Map::buildBackground() {
    mBackground = LoadTexture(backgroundFilePath);
    mBackgroundSource = {
        0.0f,
        0.0f,
        (float) mBackground.width,
        (float) mBackground.height
    };
...
void Map::renderBackground() const {
    ...
    // Second part
    Rectangle secondSource = mBackgroundSource;
    secondSource.y = mBackgroundSource.y - mBackground.height;
    DrawTexturePro(
        mBackground,
        secondSource,
        mBackgroundDestination,
...
void Map::updateBackground(float deltaTime) {
    mBackgroundScroll -= mBackgroundScrollSpeed * deltaTime;

    if (mBackgroundScroll >= mBackground.height)
        mBackgroundScroll -= mBackground.height;

    mBackgroundSource.y = mBackgroundScroll;
}
